...
def train(agent, env, dynamics_model, args,logger_kwargs=dict()):
    ...
    for i_episode in range(args.max_episodes):
        use_backup = False
        if i_episode > 3:              #Can use the backup controller from the 5th episode
        ...
        while not done:
            ...
            if (use_backup and start_using_backup):
                action = agent.select_action_backup(para_lam,obs, dynamics_model,cur_cen_pos,episode_steps)  # Use the backup controller to have the control signal
                backup_time = backup_time + 1
            else:
                action = agent.select_action(obs, dynamics_model, cur_cen_pos,warmup=args.start_steps > total_numsteps)  # Use the RL-based controller to have the control signal
            ...
            if episode_steps >= 50:
                ...
                if (start_using_backup and (not use_backup)):            # If total_difference_square is small, it means that the unicycle may get trapped in a specific position, then the backup controller is activated.
                    if total_difference_square <= 0.01:
                    ...
                    if (total_difference_square > 0.01) and (violation_time > 0):
                        violation_time = 0

                if (use_backup and start_using_backup):        # When the time to use the backup controller is longer than the threshold, or the unicycle is far enough from the position where backup controller is started (namely far enough from the trapped position), the backup controller will be stopped.
                    if backup_time >= 30:
                        use_backup = False
                        backup_time = 0
...
if __name__ == "__main__":
    ...
    exp_name = 'BLAC_Unicycle'

    logger_kwargs = setup_logger_kwargs(exp_name,args.seed,data_dir='./')

    writer = wandb.init(
        project='CBF+CLF+Augmented-Unicycle',  #   Name of the group where data are saved in wandb
        config=args,
        dir='wandb_logs',
        group='Unicycle',
    )
