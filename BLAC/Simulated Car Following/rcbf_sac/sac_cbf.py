...
class RCBF_SAC(object):
    ...
    def select_action_backup(self, para_lam,state, dynamics_model,cur_pos_vel_info, time_batch,episode_steps):
        state = to_tensor(state, torch.FloatTensor, self.device)
    ...
    def get_cbf_qp_constraints_matrix(self, dynamics_model,state_batch, action_batch, mean_pred_batch, sigma_pred_batch,lya_grads_batch,lyapunov_value,update_multipliers,previous_positions_batch,t_batch):
        ...
        if update_multipliers == False:     # According to the setting update_multipliers will be true, therefore only the later part is used.
            ...
            if self.env.dynamics_mode == 'SimulatedCars':
                ...
                # LfMfh13 = torch.bmm(torch.abs(dLfh13dx.view(batch_size, 1, -1)), fM_x.view(batch_size, -1, 1)).squeeze()
                ...
                # LfMfh15 = torch.bmm(torch.abs(dLfh15dx.view(batch_size, 1, -1)), fM_x.view(batch_size, -1, 1)).squeeze()
        ...
        if update_multipliers == True:    # The later part is used to construct part of the augmented Lagrangian function
            ...
            if self.env.dynamics_mode == 'SimulatedCars':
                ...
                # f(x) for Lyapunov derivative
                med_state = torch.zeros((state_batch.shape[0], 4)).to(self.device)
                med_state[:,0] = pos[:,2]
                med_state[:,1] = vels[:,2]
                med_state[:,2] = pos[:,3]
                med_state[:,3] = vels[:,3]
                med_state = med_state.unsqueeze(-1)

                lya_trans = torch.zeros((state_batch.shape[0], 4, 4)).to(self.device)
                lya_ele = torch.diag(torch.ones(4)).to(self.device)
                lya_ele[3, 3] = 0.0
                lya_trans[:, ] = lya_ele
                lya_mid_resu = torch.bmm(lya_trans,med_state)
                lya_resu = lya_mid_resu.squeeze(-1)
                ...
                lya_f_x[:, 2] = 0.0
                lya_f_x[:, 3] = 0.0
                lya_f_x_star = (self.env.dt * lya_f_x + lya_resu).unsqueeze(-1)
                ...
                lya_fM_x[:, 2] = mean_pred_batch[:, 6, 0].squeeze(-1)
                lya_fM_x[:, 3] = mean_pred_batch[:, 7, 0].squeeze(-1)
                lya_fM_x_star = lya_fM_x.unsqueeze(-1)
                ...
                # Make the prediction based on the dynamics
                next_batch = f_x + torch.bmm(g_x,action_batch) + mu_star

                lya_next_batch = (lya_f_x_star + torch.bmm(lya_g_x_star, action_batch) + lya_fM_x_star).squeeze(-1)

                lyapunov_value_next = self.lyapunovNet(lya_next_batch)

                Lya_value_detach = lyapunov_value.detach()

                Lya_term = ((lyapunov_value_next - Lya_value_detach) / self.env.dt) + gamma_l * Lya_value_detach
            ...
            filter = torch.zeros_like(matr)
            filtered_matr = torch.where(matr > 0, matr, filter)   # Only use the information where CBF or CLF is violated. If the result is nonpositive it means CBF or CLF is meet here and therefore this information will be abandoned. Similar to Relu function.
            ...
            other_compoenent_mean = torch.abs(torch.mean(other_compoenent))
            lya_component = torch.abs(required_matrix[-1, :] - self.cost_limit)
            ratio = float(other_compoenent_mean / lya_component)     # Try to keep a balance between CLF and CBF constraints when constructing the augmented Lagrangian function.
