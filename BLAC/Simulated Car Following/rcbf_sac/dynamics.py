...
class DynamicsModel:
    ...
    def predict_next_state(self, state_batch, u_batch, t_batch=None, use_gps=True):      #used in generate_roll_outs
    ...
    def get_dynamics(self):
        ...
        if self.env.dynamics_mode == 'Unicycle':
            ...
            def get_g(state_batch, t_batch):
                ...
                g_x[:, 6, 0] = 1.0
                g_x[:, 7, 0] = self.env.act_acc_coef / self.env.dt  # Car 4's acceleration
                # print('In dynamics, act_acc_coef:',self.env.act_acc_coef)
                g_x = g_x * self.env.dt
                return g_x
            ...
            def get_f(state_batch, t_batch):
                ...
                # Action (acceleration)
                vels_des = 3.0 * np.ones((state_batch.shape[0], 5))  # Desired velocities
                vels_des[:, 0] -= 4 * np.sin(t_batch)
                accels = kp * (vels_des - vels)
                accels[:, 1] -= k_brake * (pos[:, 0] - pos[:, 1]) * ((pos[:, 0] - pos[:, 1]) < 6.5)
                accels[:, 2] -= k_brake * (pos[:, 1] - pos[:, 2]) * ((pos[:, 1] - pos[:, 2]) < 6.5)
                accels[:, 3] = 0.0
                # accels[:, 3] -= k_brake * (pos[:, 2] - pos[:, 3]) * ((pos[:, 2] - pos[:, 3]) < 6.5)  # Car 4's acceleration is controlled directly
                ...
                f_x[:, 6] = 0.0
                f_x[:, 7] = 0.0
                trans = np.zeros((state_batch.shape[0], 10, 10))
                ele = np.diag(np.ones(10))
                ele[7, 7] = 0.0
                trans[:, ] = ele
                further_needed = np.expand_dims(state_batch, -1)
                res = np.einsum("ijk,ikn->ijn", trans, further_needed)
                new_res = res.squeeze(-1)
                f_x = self.env.dt * f_x + new_res
                # print('show the shape')
                # print(f_x.shape)
                # assert 1==2
                return f_x
    ...
    def append_transition(self, state_batch, u_batch, next_state_batch, t_batch=None):
        ...
        disturbance_batch = (next_state_batch - (self.get_f(state_batch, t_batch) + (self.get_g(state_batch, t_batch) @ u_batch).squeeze(-1)))
