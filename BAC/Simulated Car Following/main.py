...
def train(agent, env, dynamics_model, args,logger_kwargs=dict()):
    ...
    for i_episode in range(args.max_episodes):
        ...
        while not done:
            ...
            action = agent.select_action(obs, dynamics_model, cur_pos_vel_info,
...
if __name__ == "__main__":
    ...
    exp_name = 'BAC_CarsFollowing'

    logger_kwargs = setup_logger_kwargs(exp_name,args.seed,data_dir='./')

    # Need to have a wandb account to save the data
    writer = wandb.init(
        project='OnlyCBF+Augmented-SimulatedCars',  #   Name of the group where data are saved in wandb
        config=args,
        dir='wandb_logs',
        group='SimulatedCars',
    )
